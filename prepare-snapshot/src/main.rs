...
fn genesis_storage(
    nonnative_balances: Vec<(AccountId, AssetId, Balance)>,
    native_balances: Vec<(AccountId, Balance)>,
    assets: &RegistryState,
) -> TestExternalities {
...
fn load_registry_state(
    ...
    AssetId,
    AssetDetails<<FuzzedRuntime as pallet_asset_registry::Config>::StringLimit>,
)> {
    let mut result = vec![];
    ext.execute_with(|| {
        ...
        for r in registry {
            let asset =
                pallet_asset_registry::Assets::<FuzzedRuntime>::get(r).expect("asset not found");
...
fn load_balances_for_important_accounts(
    ext: &mut RemoteExternalities<hydradx_runtime::Block>,
    asset_ids: &RegistryState,
) -> Vec<(AccountId, AssetId, Balance)> {
...
fn load_native_balances(
    ...
    for acc in &accounts {
        let balance = ext
            .execute_with(|| pallet_balances::Pallet::<FuzzedRuntime>::free_balance(acc.clone()));
        if balance > 0 {
...
pub fn main() {
    ...
    let mut nonnative_balances =
        load_balances_for_important_accounts(&mut ext_mainnet, &registry_state);
    ...
    mocked_externalities.execute_with(|| {
        ...
        for (acc, asset_id, amount) in nonnative_balances {
            orml_tokens::Pallet::<FuzzedRuntime>::set_balance(
                RuntimeOrigin::root(),
                acc,
                asset_id,
                amount,
            ...
            .expect("failed to set nonative token balance");
        }
    });
    ...
    mocked_externalities.execute_with(|| {
        ...
        for r in c {
            let asset =
                pallet_transaction_multi_payment::AcceptedCurrencies::<FuzzedRuntime>::get(r);
            //println!("{:?}: {:?}", r, asset);
        }
